...
class Solution {
    ...
    // input
    // int k 명예의 전당 목록의 점수의 개수, int [] score 마지막날까지 출연한 가수들의 점수
    //
    // 계산
    // PriorityQueue 사용 높은 우선순위의 요소를 먼저 꺼내서 처리하는 구조
    // 작은 값 부터 큰값 순서로 정렬됨
    // .poll() queue 앞에 있는 값 반환후 삭제 비어있는 경우 null 반환
    // .peek() queue 의 맨 앞에 있는 값 반환 비어있는 경우 null 반환
    //
    // output
    // 최하위 점수
    public int[] solution(int k, int[] score) {
        ...
        for (int i = 0; i < score.length; i++) {
            ...
            // queue 수가 k보다 크면 제일 작은것 부터 제거
            if (queue.size() > k) {
                queue.poll();
            }

            // 제일 작은 값을 리턴만 하고 삭제는 하지않음
            answer[i] = queue.peek();
        }
        return answer;
    }
